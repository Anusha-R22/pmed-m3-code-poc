Attribute VB_Name = "basMainMACROModule"
'-----------------------------------------------------------------------------------------------'
...
'   NCJ 18 Jun 07 - Added MACRO_DU (Diagnostic Utility)
'   Mo 2/7/2007     Added MACRO_DD (Double Data Entry Module)
'                   plus minor changes in GetApplicationTitle
'   ic 23/02/2007 bug 2855 connection string is no longer required for cc check
'   Mo 28/1/2008    Bug 2979 UserRole added to the BatchDataEntry Commandline login.
'                   Changes made to BDCommandLineLoginOK & CreateBDCLLogFile
'   Mo 5/2/2008     gsEnteredPassword added for use of DoubleDataEntry module
'------------------------------------------------------------------------------------------'
Option Explicit
Option Base 0
...
'Mo 5/2/2008
Public gsEnteredPassword As String
...
#End If
    ...
    If blnLoginSucceeded Or blnShowUserInterface = False Then
        ...
        ' NCJ 18 Jun 07 - Added MACRO_DU
        'Mo Morris 2/7/2007, "MACRO_DD" , Double Data Entry module added
        Select Case App.Title
        Case "MACRO_SD", "MacroCreateDataViews", "MACRO_QM", "MACRO_SM", _
            "MACRO_BD", "MACRO_BV", "MACRO_UT", "MACRO_DU", "MACRO_DD"
        ...
        gbClinicalCoding = oVersion.HasUpgrade("", mCCSwitch)
...
Const lBATCHVALIDATION = 12
    ...
    Case Else
        ' Including Utilities and Diagnostic Utility
...
'         GetApplicationTitle = "MACRO Exchange"
    ...
    Case "MACRO_UT"
        'Mo 2/7/2007
        GetApplicationTitle = APPTITLE_UT
    Case "MACRO_CC"
        GetApplicationTitle = APPTITLE_CC
    Case "MACRO_DU"
        'Mo 2/7/2007
        GetApplicationTitle = APPTITLE_DU
    'Mo 2/7/2007
    Case "MACRO_DD"
        GetApplicationTitle = APPTITLE_DD
    Case Else
...
'---------------------------------------------------------------------
'Changed Mo Morris 28/1/2008 Bug 2979
...
Dim sMessage As String
    ...
    Case "BI"
        If UBound(asBDCLCommandParts) = 6 Then
            sBDCLUserName = asBDCLCommandParts(2)
            sBDCLPassword = asBDCLCommandParts(3)
            sBDCLDatabaseCode = asBDCLCommandParts(4)
            sBDCLRoleCode = asBDCLCommandParts(5)
            gsBDCLPathAndNameOfFile = asBDCLCommandParts(6)
        Else
    ...
    If sBDCLUserName = "" Or sBDCLPassword = "" Or sBDCLDatabaseCode = "" Or sBDCLRoleCode = "" Then
        Call CreateBDCLLogFile(sCommandLine, "Command line contains empty parameters.")
        BDCommandLineLoginOK = False
        Exit Function
    End If
    
    'Validate the UserName and Password by calling Login
    If goUser.Login(gsSecCon, sBDCLUserName, sBDCLPassword, DefaultHTMLLocation, GetApplicationTitle, sMessage, False) <> LoginResult.Success Then
        Call CreateBDCLLogFile(sCommandLine, "Could not Login using supplied UserName and Password." & vbNewLine & sMessage)
        BDCommandLineLoginOK = False
        Exit Function
    End If
    ...
    'Check that sBDCLRoleCode is a valid rolecode for the specified database
    bBDCLDBValid = False
    For Each vBDCLUserRole In goUser.UserRoles
        If sBDCLRoleCode = vBDCLUserRole Then
            bBDCLDBValid = True
        End If
    Next
    If Not bBDCLDBValid Then
        Call CreateBDCLLogFile(sCommandLine, "Supplied RoleCode is not valid for the supplied Database.")
        BDCommandLineLoginOK = False
        Exit Function
    End If
    
    'Login again, with Database and RoleCode specified
    If goUser.Login(gsSecCon, sBDCLUserName, sBDCLPassword, DefaultHTMLLocation, GetApplicationTitle, sMessage, False, sBDCLDatabaseCode, sBDCLRoleCode) <> LoginResult.Success Then
        Call CreateBDCLLogFile(sCommandLine, "Could not Login using supplied Database & RoleCode." & vbNewLine & sMessage)
        BDCommandLineLoginOK = False
        Exit Function
    End If
...
Dim nBDCLFileNumber As Integer
    ...
    Print #nBDCLFileNumber, "     /BI/UserName/Password/DatabaseCode/RoleCode/PathAndNameOfFile"
    Print #nBDCLFileNumber, "The syntax for a Batch Upload Commamd Line call is:-"
    Print #nBDCLFileNumber, "     /BU/UserName/Password/DatabaseCode/RoleCode"
    Print #nBDCLFileNumber, "------------------------------------------------------"
