...
Begin VB.Form frmMenu 
   Caption         =   "MACRO API Test"
   ClientHeight    =   8790
   ClientLeft      =   3045
   ClientTop       =   4020
   ClientWidth     =   12495
   Icon            =   "frmMenuAPITest.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   8790
   ScaleWidth      =   12495
   Begin VB.CommandButton cmdExportAssocs 
      Caption         =   "Export Assocs"
      Height          =   495
      Left            =   11040
      TabIndex        =   25
      Top             =   2040
      Width           =   1215
   End
   Begin VB.CommandButton cmdImportAssocs 
      Caption         =   "Import Assocs"
      Height          =   495
      Left            =   11040
      TabIndex        =   24
      Top             =   1320
      Width           =   1215
   End
   Begin VB.CommandButton cmdPwds 
      Caption         =   "Passwords..."
      Height          =   375
      Left            =   11040
      TabIndex        =   23
   ...
   Begin VB.Frame fraRevalidation 
      ...
      Top             =   3300
      Width           =   12015
      Begin VB.CommandButton cmdImportCats 
         Caption         =   "Import Categories"
         Height          =   375
         Left            =   7560
         TabIndex        =   22
         Top             =   240
         Width           =   1575
      End
      Begin VB.CommandButton cmdExportCats 
...
'----------------------------------------------------------------------------------------'
' File:         frmMenuAPITest.frm
' Copyright:    InferMed Ltd. 2004-2006. All Rights Reserved
' Author:       Nicky Johns, December 2004
' Purpose:      Tests the MACRO API interface
'----------------------------------------------------------------------------------------'
' Revisions:
'   NCJ 15 Dec 04 - Initial development (based on LISA test harness)
'   NCJ 9 Aug 06 - We don't use SOAP any more; added Registration & Create Subject tests
'----------------------------------------------------------------------------------------'

Option Explicit

Private Const msTEST_SUBJ_LABEL = "1"

Private mlSelStudyId As Long
Private msSelStudyName As String
Private msSelSite As String
Private msSubjLabel As String
Private msSubjId As String
...
Dim sglT2 As Single
    ...
    ' Check we want to do it
    sMsg = "Create a subject for " & msSelStudyName & "/" & msSelSite & " ?"
    If DialogQuestion(sMsg) = vbYes Then

        Screen.MousePointer = vbHourglass
        txtMsg.Text = Now & " - Creating subject, please wait..."
        DoEvents
        
        sglT1 = Timer
        ' Use DLL
        Set oAPI = New MACROAPI
        lSubjID = oAPI.CreateSubject(moUser.GetStateHex(False), mlSelStudyId, msSelSite, sMsg)
        sglT2 = Timer
        Set oAPI = Nothing
        
        txtMsg.Text = "Subject ID = " & lSubjID
        If lSubjID < 1 Then
            txtMsg.Text = txtMsg.Text & vbCrLf & sMsg
        End If
...
Exit Sub
Errlabel:
    txtMsg.Text = "An error occurred in Creating Subject!" & vbCrLf _
    ...
    If txtXMLSubject.Text > "" Then
        ...
        If DialogQuestion(sMsg) = vbYes Then
            ...
            txtMsg.Text = "Data retrieval result code = " & _
                            oAPI.GetXMLSubjectData(moUser.GetStateHex(False), _
                                GetTheXMLRequest, sRetData)
            Set oAPI = Nothing
            sglT2 = Timer
            Screen.MousePointer = vbNormal
...
'--------------------------------------------------------------------
Private Sub cmdExportAssocs_Click()
'--------------------------------------------------------------------
' Export associations
'--------------------------------------------------------------------
Dim sLogFile As String
...
Dim sglT2 As Single
    ...
    If txtXML.Text > "" Then
        ' Check they want to do it
        sMsg = "Process this XML Export Associations request?"
        If DialogQuestion(sMsg) = vbYes Then
    
            Screen.MousePointer = vbHourglass
            txtMsg.Text = Now & " - Retrieving associations, please wait..." & vbCrLf
            DoEvents
            
            sglT1 = Timer
            ' Use DLL
            Set oAPI = New MACROAPI
            txtMsg.Text = txtMsg.Text & "Export Associations result code = " & _
                            oAPI.ExportAssociations(moUser.GetStateHex(False), _
                                txtXML.Text, sRetData)
            Set oAPI = Nothing
            sglT2 = Timer
            Screen.MousePointer = vbNormal
...
Exit Sub
Errlabel:
    txtMsg.Text = "An error occurred in ExportAssociations!" & vbCrLf _
                    & Err.Number & ", " & Err.Description
    Screen.MousePointer = vbNormal
End Sub
...
'--------------------------------------------------------------------
Private Sub cmdExportCats_Click()
'--------------------------------------------------------------------
' Export categories
'--------------------------------------------------------------------
Dim sLogFile As String
...
Dim sglT2 As Single
    ...
    If txtXML.Text > "" Then
        ' Check they want to do it
        sMsg = "Process this XML category export request?"
        If DialogQuestion(sMsg) = vbYes Then
    
            Screen.MousePointer = vbHourglass
            txtMsg.Text = Now & " - Retrieving categories, please wait..."
            DoEvents
            
            sglT1 = Timer
            ' Use DLL
            Set oAPI = New MACROAPI
            txtMsg.Text = "Category export result code = " & _
                            oAPI.ExportCategories(moUser.GetStateHex(False), _
                                txtXML.Text, sRetData)
            Set oAPI = Nothing
            sglT2 = Timer
            Screen.MousePointer = vbNormal
...
Exit Sub
Errlabel:
    txtMsg.Text = "An error occurred in ExportCategories!" & vbCrLf _
                    & Err.Number & ", " & Err.Description
    Screen.MousePointer = vbNormal
End Sub

Private Sub cmdImportAssocs_Click()
'--------------------------------------------------------------------
' Import associations
'--------------------------------------------------------------------
Dim sLogFile As String
...
Dim lRetCode As Long
    ...
    If txtXML.Text > "" Then
        ' Check they want to do it
        sMsg = "Process this XML Import Associations request?"
        If DialogQuestion(sMsg) = vbYes Then
    
            Screen.MousePointer = vbHourglass
            txtMsg.Text = Now & " - Importing associations, please wait..." & vbCrLf
            DoEvents
            
            sglT1 = Timer
            ' Use DLL
            Set oAPI = New MACROAPI
            lRetCode = oAPI.ImportAssociations(moUser.GetStateHex(False), _
                                txtXML.Text, sRetData)
            txtMsg.Text = txtMsg.Text + "Import Associations result code = " & lRetCode
            Set oAPI = Nothing
            sglT2 = Timer
            Screen.MousePointer = vbNormal
        ...
        End If
    Else
        MsgBox "No XML specified!"
    End If
    
Exit Sub
Errlabel:
    txtMsg.Text = "An error occurred in ImportAssocs!" & vbCrLf _
                    & Err.Number & ", " & Err.Description
    Screen.MousePointer = vbNormal
...
Private Sub cmdImportCats_Click()
'--------------------------------------------------------------------
' Export categories
'--------------------------------------------------------------------
Dim sLogFile As String
...
Dim sglT2 As Single
    ...
    If txtXML.Text > "" Then
        ' Check they want to do it
        sMsg = "Process this XML category import request?"
        If DialogQuestion(sMsg) = vbYes Then
    
            Screen.MousePointer = vbHourglass
            txtMsg.Text = Now & " - Importing categories, please wait..."
            DoEvents
            
            sglT1 = Timer
            ' Use DLL
            Set oAPI = New MACROAPI
            txtMsg.Text = "Category import result code = " & _
                            oAPI.ImportCategories(moUser.GetStateHex(False), _
                                txtXML.Text, sRetData)
            Set oAPI = Nothing
            sglT2 = Timer
            Screen.MousePointer = vbNormal
...
Exit Sub
Errlabel:
    txtMsg.Text = "An error occurred in ImportCats!" & vbCrLf _
                    & Err.Number & ", " & Err.Description
    Screen.MousePointer = vbNormal

End Sub

Private Sub cmdPwds_Click()

    Call frmPasswords.Display(moUser.GetStateHex(False), msSecurityDB)
...
Dim sUser As String
    ...
    ' Check they want to do it
    sMsg = "Register subject " & msSelStudyName & "/" & msSelSite & "/" & msSubjLabel & " ?"
    If DialogQuestion(sMsg) = vbYes Then
    
        Screen.MousePointer = vbHourglass
        txtMsg.Text = Now & " - Doing registration, please wait..."
        DoEvents
        ...
        ' Use DLL
        Set oAPI = New MACROAPI
        txtMsg.Text = txtMsg.Text & vbCrLf _
                        & "Finished Registration" & vbCrLf _
                        & "Result code = " _
                        & oAPI.RegisterSubject(sUser, msSelStudyName, msSelSite, msSubjLabel, sRetData)
        Set oAPI = Nothing
        ...
        txtMsg.Text = txtMsg.Text & vbCrLf _
                    & "Reg ID = " & sRetData & vbCrLf _
                    & "Time taken = " & sglT2 - sglT1
    End If
    ...
    If txtXMLSubject.Text > "" Then
        ...
        If DialogQuestion(sMsg) = vbYes Then
            ...
            txtMsg.Text = "Finished Data Input" & vbCrLf _
                            & "Result code = " _
                            & oAPI.InputXMLSubjectData(sUser, GetTheXMLRequest, sRetData)
            Set oAPI = Nothing
...
'    txtXML.Text = ""
    ...
    ' Do MACRO login
    lLoginResult = frmAPILogin.Display(sMsg, sUserFull, sSerialUser, msSecurityDB)
    ...
    Else
        ...
        If lLoginResult = 0 And sSerialUser > "" Then
            ...
            txtMsg.Text = txtMsg.Text & "MACRO DB = " & moUser.DatabaseCode & vbCrLf
            Call InitialiseMe
        Else
...
'    cmdRefresh.Left = lstSubjects.Left + lstSubjects.Width - cmdRefresh.Width
    ...
    cmdRegister.Left = cmdXMLInput.Left - cmdRegister.Width - mlGAP
    cmdExportCats.Left = cmdRegister.Left - cmdExportCats.Width - mlGAP
    cmdImportCats.Left = cmdExportCats.Left - cmdImportCats.Width - mlGAP
...
'    If (msSubjLabel > "") Then
        lblSubject.Caption = msSelStudyName & "/" & msSelSite & "/" & msSubjLabel
        Call DoSubjectXML
